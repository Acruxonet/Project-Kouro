...
OBS_STATE_DIM = 20  # base_pos(3) + base_rot6d(6) + ee_pos_rel(3) + ee_rot6d(6) + gripper_qpos(2)
...
def quaternion_xyzw_to_rotation_6d(quaternion: np.ndarray) -> np.ndarray:
    """Convert one xyzw quaternion to the first two columns of its rotation matrix."""
    quat = np.asarray(quaternion, dtype=np.float64)
    if quat.shape != (4,):
        raise ValueError(f"Expected quaternion shape (4,), got {quat.shape}")
    norm = np.linalg.norm(quat)
    if norm < 1e-12:
        raise ValueError("Cannot convert a zero-norm quaternion to rotation 6D")
    x, y, z, w = quat / norm
    return np.asarray(
        [
            1.0 - 2.0 * (y * y + z * z),
...
class RoboCasaEnv(gym.Env):
    ...
    def _format_raw_obs(self, raw_obs: dict) -> RobotObservation:
        ...
        # The source quaternions are xyzw. Convert both rotations to the
        # continuous 6D representation used by the training dataset cache.
        identity_quat = np.asarray([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
        base_rotation_6d = quaternion_xyzw_to_rotation_6d(
            raw_obs.get("state.base_rotation", identity_quat)
        )
        ee_rotation_6d = quaternion_xyzw_to_rotation_6d(
            raw_obs.get("state.end_effector_rotation_relative", identity_quat)
        )
