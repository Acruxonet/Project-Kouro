...
class DiffusionConfig(PreTrainedConfig):
    ...
    # Match the upstream Diffusion Policy sampling default while retaining
    # near-terminal observations from short demonstrations.
    ...
    def __post_init__(self):
        ...
        if not 1 <= self.n_action_steps <= max_action_steps:
            raise ValueError(
                "`n_action_steps` must be in [1, horizon - n_obs_steps + 1]. "
                f"Got {self.n_action_steps=} for {self.horizon=} and {self.n_obs_steps=}."
            )
