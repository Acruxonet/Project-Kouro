...
class PI05Policy(PreTrainedPolicy):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Load state dict (expects keys with "model." prefix). Fine-tuning must
        # fail closed: a missing or incompatible checkpoint must never silently
        # turn this run into training a randomly initialized 4B model.
        try:
            print(f"Loading pretrained weights from: {pretrained_name_or_path}")
            try:
                ...
                raise RuntimeError(
                    f"Could not load pretrained Pi05 weights from {pretrained_name_or_path}"
                ) from e
            ...
            raise RuntimeError(
                f"Pi05 pretrained checkpoint validation/loading failed: {pretrained_name_or_path}"
            ) from e
