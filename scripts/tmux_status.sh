...
run_id=$(tmux show-option -qv -t "$session_name" @kouro_run_id)
experiment_dir=$(tmux show-option -qv -t "$session_name" @kouro_experiment_dir)

printf 'session: %s\n' "$session_name"
printf 'experiment: %s\n' "${run_id:-unknown}"
tmux list-windows -t "$session_name" -F 'window=#{window_index}:#{window_name} active=#{window_active} dead=#{pane_dead} command=#{pane_current_command}'
...
if tmux list-windows -t "$session_name" -F '#{window_name}' | grep -qx train; then
    output_window=train
...
tmux capture-pane -p -t "$session_name:$output_window" -S -30 | tail -30
